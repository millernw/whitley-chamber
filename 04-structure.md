...
**Header nav:**
- **Style:** Sticky top bar — transparent over the hero, becomes solid navy (`#082F5B`) with white text on scroll
- **Logo position:** Left — the Open Door full logo (`Open_Door_Brand_Full_Logo_Transparent.png`/`.svg`); use the white version (`Open_Door_Brand_Full_Logo_White.png`) once the bar turns solid navy
- **Nav links (desktop):** Why Join | Member Benefits | Member to Member Deals | Events | About | Contact
- **CTA in nav:** Yes — "Join the Chamber" button in brick red (`#B32120`) with white text, always visible
- **Mobile nav:** Hamburger menu → full-screen overlay in navy with white links and a brick-red CTA button
...
1. **Hero**
   - Layout: Full-bleed background photo of Whitley County (Columbia City courthouse, a main-street storefront, or a path/doorway scene), navy-to-transparent gradient overlay for legibility. Wide-tracked uppercase tagline eyebrow ("Opening Doors to Whitley County"), large Lora headline left-aligned, DM Sans subheadline below, two CTA buttons (primary brick red, secondary navy/outline) stacked on mobile / side by side on desktop. Subtle parallax on background image.
...
2. **Trust Bar / Member Logos**
   - Layout: Full-width band in light cool gray (`#F4F6F9`). Marquee/horizontal scroll of current member business logos. Caption above: "Proudly serving Whitley County businesses." No hard sell — just proof of presence.
...
4. **Why Join — Member Benefits**
   - Layout: 3-column card grid on desktop, 1-column on mobile. Each card: Phosphor icon (navy, or brick red for emphasis), bold short headline, 2-sentence FAB description. Cards use subtle hover lift animation.
   - Formula: FAB
   - Goal: Communicate the five core membership benefits clearly and quickly

5. **Member to Member Deals — Feature Callout**
   - Layout: Full-bleed section with navy background (`#082F5B`), white typography with brick-red accents (headline emphasis, underline, key numbers). Left side: bold headline + copy. Right side: styled membership card mockup graphic. CTA button in brick red.
...
7. **Testimonials**
   - Layout: 3-column card carousel on desktop (manual navigation, swipe on mobile). Each card: large opening quote mark in brick red, quote text in Lora italic, member name and business in DM Sans below. Cards on white with a light cool-gray (`#F4F6F9`) surface and subtle border.
...
8. **Stats / Social Proof Band**
   - Layout: Full-width band in navy (`#082F5B`). 3 large animated counter stats side by side, the numerals in brick red (`#B32120`) with white labels below: member count, years serving the community, annual events. (Navy band, not a red wash — red is reserved for the numerals and accents.)
...
9. **Pricing — Membership Tiers**
   - Layout: Clean table layout on desktop. Alternating white and light cool-gray (`#F4F6F9`) rows. Business size in one column, annual price in the other, short "who it's for" description in a third. Anchor note below table: "Starting at less than $8/month." CTA button (brick red) below table.
...
10. **FAQ**
    - Layout: Accordion, single column, max-width 800px centered. One open at a time. Light cool-gray (`#DCE1E8`) divider lines. Question in DM Sans semi-bold, answer in regular weight.
...
11. **Final CTA Band**
    - Layout: Full-bleed section with deep navy (`#082F5B`) background and large Lora headline centered. Single brick-red CTA button. Optional: a subtle open-door/path line motif behind the headline. No other distractions.
...
- **Column 1 — Brand:** Open Door logo (white version, `Open_Door_Brand_Full_Logo_White.png`, on the navy footer) + tagline "Opening Doors to Whitley County" + short description (1 sentence). Social media icons below (Facebook, Instagram, LinkedIn — confirm active accounts with Chamber).
...
- **Bottom bar:** © 2026 Whitley County Chamber of Commerce. All rights reserved. | Privacy Policy | Terms of Service *(legal name used here in the copyright line by design; the display brand "The Chamber of Whitley County" is used everywhere else)*
...
- **Back to top button:** Yes — appears after scrolling past the hero, bottom-right, brick red (`#B32120`)
